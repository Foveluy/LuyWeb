...
class LuyA_Test():
    ...
    async def _request(self, url='/', method='get', headers=None, data=None):
        conn = aiohttp.TCPConnector(verify_ssl=False)
        async with aiohttp.ClientSession(connector=conn) as session:
            async with getattr(
                    session, method.lower())('http://127.0.0.1:{}'.format(PORT) + url, headers=headers, data=data) as response:
                try:
    ...
    def _run_test_server(self, url='/', method='get', headers=None, data=None):

        rsp = [None]

        async def request():
            rsp[0] = await self._request(url=url, method=method, headers=headers, data=None)
            self.app.stop()

        self.app.listener('after_start')(request)

        self.app.run(port=PORT, debug=False)
        self.app.after_server_start.pop()  # have to pop the listener
        return rsp[0]
